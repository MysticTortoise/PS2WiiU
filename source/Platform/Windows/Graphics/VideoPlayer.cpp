...
#include "Windows/Graphics/PlatformVideoPlayer.hpp"
...
TeaPacket::Graphics::PlatformVideoPlayer::PlatformVideoPlayer(const std::string path){
...
void TeaPacket::Graphics::VideoPlayer::UpdateFrame(){
    ...
    while(av_read_frame(platformPlayer->formatContext, platformPlayer->avPacket) >= 0){
        if(platformPlayer->avPacket->stream_index != platformPlayer->streamID) { continue; }
        int result = avcodec_send_packet(platformPlayer->codecContext, platformPlayer->avPacket);
        if(result < 0){
        ...
        result = avcodec_receive_frame(platformPlayer->codecContext, platformPlayer->avFrame);
        if(result < 0){
    ...
    if(!foundFrame && loop){
        av_seek_frame(platformPlayer->formatContext, platformPlayer->streamID, 0, AVSEEK_FLAG_FRAME);
        UpdateFrame();
        return;
    }

    sws_scale(platformPlayer->swsContext, platformPlayer->avFrame->data, platformPlayer->avFrame->linesize, 0, platformPlayer->codecContext->height, platformPlayer->texFrame->data, platformPlayer->texFrame->linesize);
    texture->UpdateContents(platformPlayer->texFrame->data[0]);
}
...
TeaPacket::Graphics::VideoPlayer::~VideoPlayer(){
    ...
    av_packet_unref(platformPlayer->avPacket);
    av_free(platformPlayer->internalBuffer);
    av_frame_free(&platformPlayer->texFrame);
    av_free(platformPlayer->texFrame);
    av_frame_free(&platformPlayer->avFrame);
    av_free(platformPlayer->avFrame);
    avcodec_free_context(&platformPlayer->codecContext);
    avformat_close_input(&platformPlayer->formatContext);

    av_free(platformPlayer->customIOStream.avioContext);
